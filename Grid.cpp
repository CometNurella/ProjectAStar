...
namespace astar
{
    Grid::Grid(std::size_t width, std::size_t height)
        : m_width{ width },
        m_height{ height },
        m_walkable(width* height, true)
    {
    }

    std::size_t Grid::getNodeCount() const noexcept
    {
    ...
    void Grid::setWalkable(int x, int y, bool walkable)
    {
        if (!isInside(x, y))
        {
            throw std::out_of_range{
                "Grid coordinate is outside the grid"
            };
        }
    ...
    NodeId Grid::toNodeId(int x, int y) const
    {
        ...
        return static_cast<NodeId>(y) * m_width
            + static_cast<NodeId>(x);
    }
    ...
    std::vector<Edge> Grid::getEdges(NodeId node) const
    {
        ...
        for (const GridNode& candidate : candidates)
        {
            ...
            edges.push_back(
                Edge{
                    toNodeId(candidate.x, candidate.y),
                    1.0
                }
            );
        }
